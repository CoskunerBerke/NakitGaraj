...
(async () => {
  ...
  try {
    ...
    const modalFirstName = page.getByTestId('welcome-first-name');
    if (await modalFirstName.isVisible({ timeout: 2000 }).catch(() => false)) {
      console.log('Filling Welcome Contact Modal...');
      await modalFirstName.fill('Test');
    ...
    await page.waitForSelector('[data-testid="brand-option-bmw"]', { timeout: 10000 });
    await page.getByTestId('brand-option-bmw').click();
    ...
    // Select Engine (316i)
    const option316iVal = await engineSelect.evaluate((sel: HTMLSelectElement) => {
    ...
    const optionTrimVal = await trimSelect.evaluate((sel: HTMLSelectElement) => {
